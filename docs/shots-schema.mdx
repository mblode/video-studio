...
appended after every hand-authored reference, and a reference already pointing
at the sheet's path is adopted in place rather than duplicated — so an
`@Image 2` you typed keeps meaning the same reference and a frame role keeps
`@Image 1`.

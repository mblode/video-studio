...
| `vs narrate <lines.tsv>` | Gemini 3.8 TTS per-line VO (`NN\ttext` TSV) |
...
| `GEMINI_API_KEY` | Nano Banana stills, `vs score`, `vs narrate` |
| `GEMINI_TTS_VOICE` (optional) | Default `vs narrate` voice (else `Charon`) |

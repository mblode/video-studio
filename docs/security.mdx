...
**Keys are read from the environment only.** `AI_GATEWAY_API_KEY` (default Seedance 2.5), `ARK_API_KEY` (BytePlus), and the optional `GEMINI_API_KEY`, `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID`, and `MINIMAX_API_KEY` are loaded from `.env` or the process environment. They are never written to a manifest, log line, or error message. `.env` is gitignored; `.env.example` is the canonical list of names.
